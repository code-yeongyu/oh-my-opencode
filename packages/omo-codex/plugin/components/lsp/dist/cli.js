...
import { delimiter as delimiter2, join as join2, win32 } from "node:path";
...
function killProcessTree(proc, signal) {
  if (process.platform === "win32" && proc.pid) {
    const taskkill = [process.env["SystemRoot"], process.env["WINDIR"]].filter((root) => !!root && win32.isAbsolute(root) && win32.parse(root).root.length > 1).map((root) => win32.join(root, "System32", "taskkill.exe")).find((path2) => existsSync3(path2));
    if (taskkill) {
